...
function buildAnalysisPrompt(input: {
  ...
  return [
    ...
    "Discord is not Telegram. Treat upstream Telegram changes as a reference implementation only when they reveal a core Takopi contract, transport expectation, or user workflow this Discord transport should also support.",
    "",
    "Translate concepts into Discord-native equivalents:",
    "- Telegram chat/topic defaults map to Discord channel/thread preferences and inherited channel defaults.",
    "- Telegram commands map to Discord slash commands, interaction responses, followups, ephemeral replies, and command registration.",
    "- Telegram message ids map to Discord channel/thread/message ids and MessageRef metadata such as sender_id/raw when needed by plugins.",
    "- Telegram file sends map to Discord attachment uploads, which are constrained by Discord account/server upload caps; default outbound /file get downloads should be conservative unless configurable.",
    "- Telegram topic/session recovery maps to Discord thread/channel context and stored session bindings.",
    "- Do not copy Telegram names, UX, persistence keys, or topic assumptions directly unless they already match local Discord patterns.",
    "",
    ...
    "When a change is represented only in upstream Telegram code, explain the underlying Takopi contract it demonstrates and the Discord-specific adaptation, not a straight port.",
...
function buildImplementationPrompt(input: {
  ...
  return [
    ...
    "- Prefer existing Discord transport patterns over copying Telegram structure.",
    "- Translate Telegram reference behavior into Discord-native slash command, thread/channel, interaction, preference, and attachment behavior.",
    "- Preserve Discord-specific limits and UX: ephemeral error replies, followups after deferred interactions, thread/channel context, and configurable file transfer caps.",
