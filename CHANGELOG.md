...
### Fixed
- Replies inside a Discord thread now resume the saved session under the new default. Previously the `session_mode == "chat"` gate skipped the lookup, so each reply started fresh.
...
- Discord thread messages suppress redundant visible resume-token and `ctx:` footer lines because the parent channel/thread name already carry project and branch context.
