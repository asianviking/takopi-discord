...
| Plugin commands | Upstream plugin command surface | Register plugin commands as Discord slash commands | Partial | Slash command registration works, but channel-level plugin invocations do not yet create a new thread like engine commands/message prompts. |
...
| Directive-driven context update | Directives inside a topic can update topic binding and rename the topic | Existing Discord threads should probably update thread branch binding and rename the thread when directives resolve a new context | Partial | New Discord threads can persist directive context, but existing threads do not yet persist directive context changes. |
...
| --- | --- | --- | --- |
| Existing-thread directive context updates | Partial | If a message inside a thread resolves directives to a new project/branch, keep the parent channel project rule, update only the thread branch when appropriate, prepare/reuse the worktree, and rename the thread. | Telegram persists directive context updates into topic state and renames the topic. Discord only persists directive context for newly-created threads today. |
| Plugin slash commands in project channels | Partial | When a plugin slash command is invoked in a bound project channel, create a new thread before dispatching, matching engine slash commands and message prompts. | Current plugin slash command dispatch posts and runs in the channel unless already inside a thread. |
