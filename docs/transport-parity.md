...
| Session continuation | Continue in the same topic | Continue in the same Discord thread | Done | `session_mode` has two canonical Discord modes: `stateless` (no server-side resume, reply-chain only) and `thread` (default: server-side resume inside threads, channels stay reply-chain). Legacy `chat` config is accepted as an alias for `thread`. Within a thread, all participants share one session. |
