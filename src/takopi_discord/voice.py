"""Voice chat support for Discord transport."""
...
class VoiceSink(discord.sinks.Sink):
    """Custom sink to capture audio from voice channel users."""
    ...
    def write(self, data: bytes, user: int) -> None:
        """Called by discord.py with audio data from a user."""
        self._voice_manager.receive_audio(self._guild_id, user, data)
...
class VoiceManager:
    ...
    async def join_channel(
        ...
        voice_client.start_recording(sink, self._on_recording_finished, guild_id)
    ...
    async def leave_channel(self, guild_id: int) -> None:
        ...
        if should_delete:
            voice_channel = self._bot.get_channel(voice_channel_id)
    ...
    def receive_audio(self, guild_id: int, user_id: int, data: bytes) -> None:
    ...
    def _on_recording_finished(self, sink: VoiceSink, guild_id: int) -> None:
        """Called when recording finishes (usually on disconnect)."""
        logger.debug("Recording finished for guild %s", guild_id)
    ...
    async def handle_voice_state_update(
        ...
        if before.channel and before.channel.id == session.voice_channel_id:
            ...
            voice_channel = self._bot.get_channel(session.voice_channel_id)
